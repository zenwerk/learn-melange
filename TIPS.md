...
### `I.acceptable` による入力ヒント生成（IDE補完の基礎）

incremental API には `I.acceptable : 'a checkpoint -> token -> position -> bool` という関数がある。これは「このチェックポイントで、このトークンを渡したら受理されるか？」を判定する。`InputNeeded` 状態の checkpoint に対して全トークンを仮投入することで、次に入力可能なトークンの集合が得られる。

**重要**: これは `--inspection` フラグ不要。`--table` だけで利用できる。`--inspection` はさらに LR 状態の内部構造にアクセスするためのもので、補完程度であれば `acceptable` で十分。

#### パターン: EOF 手前で checkpoint を検査する

入力途中の文字列に対してヒントを出すには、「EOF を渡す前にパースを止めて、その時点の checkpoint で `acceptable` を問い合わせる」という流れになる。

```ocaml
(* feed_tokens の戻り値型: EOF 到達を特別扱いする *)
type 'a feed_result =
  | Fed of 'a
  | AtEof of Ast.statement I.checkpoint  (* EOF 手前の checkpoint を保持 *)
  | FeedError of string * int

let rec feed_tokens lexbuf cp =
  match cp with
  | I.InputNeeded _ ->
    let token = Lexer.token lexbuf in
    if token = Parser.EOF then
      AtEof cp                          (* ここで EOF を offer せずに返す *)
    else
      feed_tokens lexbuf (drain (I.offer cp (token, startp, endp)))
  | I.Accepted v -> Fed v
  | I.HandlingError _ | I.Rejected -> FeedError (...)
  | _ -> feed_tokens lexbuf (drain cp)

(* ヒント生成: 各トークンの代表値を acceptable でテスト *)
let all_tokens = [
  (Parser.FLOAT 0.0, "number");       (* ペイロード付きトークンはダミー値で OK *)
  (Parser.IDENT "", "identifier");    (* acceptable は「種類」だけ見る *)
  (Parser.PLUS, "+"); ...
]

let get_acceptable_tokens input =
...
- `FLOAT 0.0` や `IDENT ""` のような**ダミー値**で問題ない。`acceptable` はトークンの「種類」（コンストラクタ）だけを見る
- `feed_tokens` を分けたのは、通常のパース（`push_parse`）と EOF 手前で止めるヒント生成で、EOF 到達時の処理が異なるため。戻り値を ADT にして呼び出し側が分岐を決めるパターンは再利用性が高い
- 結果として、LR(1) 文法の先読みのおかげで文脈依存のヒントが自動的に得られる。例: `x` の後は `+ - * / = EOF`（identifier の後のみ `=` が出現）

#### IDE 補完との関係

`acceptable` は**構文レベル**のヒントしか提供しない。定義ジャンプ・型情報・スコープ解析などは別途意味解析が必要で、Menhir の守備範囲外。本格的な LSP を作るなら、`acceptable` でトークン候補を得た上で、自前の意味解析レイヤーを重ねる設計になる。
...
### OCaml オブジェクト型（`< ... > Js.t`）とレコードの違い

`%mel.obj` が生成する値の型は OCaml の**レコード型**ではなく、**オブジェクト型** `< ... > Js.t` になる。これは JS オブジェクトと直接対応させるための設計。
...
- `Js.t` — 「これは JS オブジェクトだよ」とコンパイラに伝えるラッパー
- `< ... >` — OCaml のオブジェクト型（レコードではない）。メソッド名と型のシグネチャを列挙
- 各フィールドはメソッドとして扱われ、JS 側ではプロパティアクセスになる

**なぜレコードじゃだめか:**

OCaml のレコードは内部表現（タグ付きブロック）を持つため、JS から見ると配列のようなオブジェクトになってしまい、`obj.name` のような自然なアクセスができない。一方、OCaml のオブジェクト型は `%mel.obj` と組み合わせることで、そのまま JS オブジェクトリテラルにコンパイルされる。

**関数を持つオブジェクト型:**

```ocaml
(* セッションオブジェクトの型 *)
< eval : string -> result_obj;
  hints : string -> string array > Js.t
```

→ JS 側:
```js
{
  eval: (input) => resultObj,
  hints: (input) => [...]
}
```

**ハマりポイント:** `%mel.obj { ... }` のフィールドに複数行の式（`let ... in` や `if/else`）を書くと、セミコロンがシーケンス演算子として解釈されてエラーになる。各フィールドの値を括弧で明示的に包む:
...
[%mel.obj {
  eval = fun input ->
    let s = trim input in
    if s = "" then make_error "empty" else eval_input s;   (* この ; でエラー *)
  hints = fun input -> ...
}]
...
[%mel.obj {
  eval = (fun input ->
    let s = trim input in
    if s = "" then make_error "empty" else eval_input s);
  hints = (fun input -> ...)
}]
```
